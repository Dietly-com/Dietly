...
function ProductNutrients(props) {
    ...
    useEffect(()=>{
        ...
        for (const productNutrient of props.data.productNutrients) {
            effectRows.push(createRow(productNutrient.nutrient.viewName, productNutrient.quantity, productNutrient.quantity*(props.data.quantity/100), productNutrient.nutrient.unit.viewName));
        }
        setRows(effectRows);
    }, [])
